...
/**
 * Role Solar System — an original interactive orbital diagram (design spec
 * §4.11). Every operational role orbits a central "ValuXpert Core" at its
 * own radius and speed; clicking a planet selects that role, wiring
 * straight into the same activeRoleId state the role list below already
 * uses. Built with layered CSS transforms rather than a 3D library so it
 * stays light, accessible, and legible at any viewport.
 */
  ...
  return (
    <div className="relative mx-auto flex h-[420px] w-full max-w-[560px] items-center justify-center sm:h-[520px]">
      ...
      <div className="relative z-10 flex h-20 w-20 flex-col items-center justify-center rounded-full bg-gradient-to-br from-brand-400 to-brand-600 text-center shadow-glow sm:h-24 sm:w-24">
        <span className="text-[9px] font-semibold uppercase tracking-wide text-white/90">ValuXpert</span>
        <span className="text-[9px] font-semibold uppercase tracking-wide text-white/70">Core</span>
      </div>
                ...
                >
                  ...
                  <span
                    className={`whitespace-nowrap rounded-full px-2 py-0.5 text-[9px] font-semibold uppercase tracking-wide transition-colors duration-300 ${
                      isActive ? 'bg-accent-action/20 text-accent-action' : 'text-ink-dark/40 group-hover:text-ink-dark/70'
                    }`}
                  >
                    {role.name}
                  </span>
