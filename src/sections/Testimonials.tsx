          ...
          {testimonials.map((t, i) => (
            <div
              ...
              className={`relative flex flex-col gap-6 overflow-hidden rounded-card p-8 md:p-9 ${
                i % 2 === 0 ? 'bg-surface-dark1 text-white lg:mt-0' : 'bg-brand-500 text-white lg:mt-8'
              }`}
            >
              <Quote className="absolute right-6 top-6 opacity-15" size={44} />
              <p className="text-eyebrow uppercase text-white/60">{t.company}</p>
              <p className="text-h3 !text-xl font-medium leading-snug">&ldquo;{t.quote}&rdquo;</p>
