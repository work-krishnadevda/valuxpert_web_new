...
/**
 * Role Infrastructure and the Interactive Pipeline Map are specified as one
 * wired experience (design spec §4.8/§4.9): selecting a role highlights its
 * matching node below, reinforcing "one system, one source of truth."
 */
  ...
  const nodes = useMemo(
    () =>
      pipelineNodes.map((node) => ({
        ...node,
        status: node.id === activeNodeId ? ('active' as const) : node.status === 'active' ? ('passed' as const) : node.status,
      })),
    [activeNodeId]
  );
  ...
  return (
    <section id="pipeline" className="section-pad bg-surface-light1">
      <Container className="flex flex-col gap-4">
        ...
        <h2 className="text-h2 text-balance text-ink-light">Precision Role-Based Infrastructure</h2>
        <p className="max-w-2xl text-body text-ink-muted">
          ValuXpert enforces distinct, cryptographic authorization boundaries conforming to standard operating
          procedures. Select a role to test its real workspace duties.
        </p>
      </Container>
      ...
      <Container className="mt-10">
            ...
            return (
              ...
              >
                <div className="flex items-start gap-3 sm:items-center">
                  <span className={`h-2.5 w-2.5 shrink-0 rounded-full ${isActive ? 'bg-accent-action' : 'bg-brand-500/40'}`} />
                  <div>
                    <p className={`text-nav font-semibold ${role.isAdmin ? 'text-accent-actionDark' : 'text-ink-light'}`}>
                      {role.name}
                    </p>
                    <p className="mt-0.5 max-w-xl text-sm text-ink-muted">{role.description}</p>
                  </div>
                </div>
                <div className="flex shrink-0 items-center gap-3 pl-6 sm:pl-0">
                  ...
                  <span
                    className={`rounded-control px-4 py-2 text-xs font-semibold ${
                      isActive ? 'bg-brand-500 text-white' : 'border border-ink-light/15 text-ink-light'
                    }`}
                  >
        ...
        <div className="relative overflow-hidden rounded-card bg-surface-dark1 p-6 md:p-8">
          ...
          <div className="relative mb-8 flex flex-col items-center gap-2 text-center">
            <p className="text-nav font-semibold text-white">Role Solar System</p>
            <p className="max-w-md text-xs text-ink-dark/60">
              Every role orbits the same operational core. Click a planet to switch the active workspace below.
            </p>
            <RoleSolarSystem roles={systemRoles} activeRoleId={activeRoleId} onSelect={setActiveRoleId} />
          </div>
          ...
          <div className="relative mb-6 flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-nav font-semibold text-white">Interactive Operational Pipeline Map</p>
                <p className="text-xs text-ink-dark/60">Standard Operating Procedure (SOP) path</p>
              </div>
            </div>
            <button
              type="button"
              ...
              className={`rounded-full px-3 py-1.5 text-xs font-semibold transition-colors ${
                liveFlow ? 'bg-brand-500/20 text-brand-400' : 'bg-white/10 text-ink-dark/50'
              }`}
            >
          ...
          <p className="mb-6 max-w-2xl text-sm text-ink-dark/60">
            The flowchart represents the sequential compliance pipeline of ValuXpert. Data flows in a single secure
            trajectory from configuration to banking credit dispatch.
          </p>
            ...
            {nodes.map((node) => (
              ...
              >
                <div className="flex items-center justify-between">
                  <span className="font-mono text-[10px] text-ink-dark/50">{node.code}</span>
                  {node.status === 'active' ? (
                    <span className="h-1.5 w-1.5 animate-pulse rounded-full bg-brand-400" />
                  ) : null}
                </div>
                <Icon name={node.icon} className={node.status === 'active' ? 'text-brand-400' : 'text-ink-dark/60'} />
                <p className="text-xs font-semibold leading-tight text-white">{node.title}</p>
                <span
                ...
                >
                  {node.status === 'active' ? 'Active Work' : node.status === 'passed' ? 'Passed ✓' : 'Queued'}
                </span>
              </div>
            ))}
          </div>
